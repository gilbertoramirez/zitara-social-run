          ...
          {EVENT.schedule.map((item, i) => (
            <div key={i} className="flex gap-6 mb-12 last:mb-0">
              <div className="flex flex-col items-center">
                <div className="w-14 h-14 rounded-full bg-zitara-olive flex items-center justify-center shrink-0">
                  <span className="text-white text-xs font-bold">{item.time}</span>
                </div>
                {i < EVENT.schedule.length - 1 && (
                  <div className="w-0.5 h-full bg-zitara-gold/30 mt-2" />
                )}
              </div>
              <div className="pt-2">
                <h3 className="text-xl font-bold text-zitara-olive">
                  {item.title}
                </h3>
